...
app.get('/api/productos-no-vendidos', async(req, res) => {
    try {
        const result = await pool.query(`
            SELECT 
                id_producto,
...
app.get('/api/estadisticas-clientes', async(req, res) => {
    try {
        const result = await pool.query(`
            SELECT 
                c.id_cliente,
...
app.get('/api/productos-top', async(req, res) => {
    try {
        const result = await pool.query(`
            SELECT 
                p.id_producto,
...
app.get('/api/ventas-por-categoria', async(req, res) => {
    try {
        const result = await pool.query(`SELECT * FROM vista_ventas_por_categoria`);
        res.json(result.rows);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.get('/api/clientes', async(req, res) => {
    try {
        const result = await pool.query('SELECT id_cliente, nombre, apellido FROM cliente ORDER BY nombre');
        res.json(result.rows);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.get('/api/empleados', async(req, res) => {
    try {
        const result = await pool.query('SELECT id_empleado, nombre, apellido FROM empleado ORDER BY nombre');
        res.json(result.rows);
    } catch (err) {
        res.status(500).json({ error: err.message });
    }
...
app.get('/api/productos-lista', async(req, res) => {
    try {
        const result = await pool.query(`
...
app.get('/api/categorias-stats', async(req, res) => {
    try {
        const result = await pool.query('SELECT COUNT(*) as total FROM categoria');
